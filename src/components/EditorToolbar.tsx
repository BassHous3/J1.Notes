...
export default function EditorToolbar({ editor }: EditorToolbarProps) {
  ...
  const Divider = () => <div className="w-px h-5 mx-0.5" style={{ backgroundColor: 'var(--theme-border)' }} />;
  ...
  // On regular-sized screens there's room for everything in one row — only
  // condense down to a "basic + expandable" layout when the viewport is
  // actually too narrow to fit it all (isSmallScreen).
  if (!isSmallScreen) {
    return (
      <div
  ...
  return (
    <div
      ...
      style={{ borderColor: 'var(--theme-border)', backgroundColor: 'var(--theme-hover)' }}
    >
      {/* Condensed row — always visible: Bold/Italic/Underline/Strike + colors */}
      <div className="flex items-center gap-0.5 px-2 py-1">
        <BasicButtons />
        <Divider />
        <ColorPickers />

        {/* Expand / collapse the rest of the formatting options */}
        <button
          onMouseDown={e => { e.preventDefault(); setShowMore(m => !m); }}
          className="p-1.5 rounded transition-colors ml-auto flex items-center justify-center"
          style={{ color: 'var(--theme-text-muted)' }}
          title={t('editor.moreFormatting', 'More formatting')}
          onMouseEnter={e => (e.currentTarget.style.backgroundColor = 'var(--theme-hover)')}
          onMouseLeave={e => (e.currentTarget.style.backgroundColor = 'transparent')}
        >
          <ChevronDown size={16} className="transition-transform" style={{ transform: showMore ? 'rotate(180deg)' : 'none' }} />
        </button>
      </div>

      {/* Expanded row — everything else, shown only when toggled open */}
      {showMore && (
        <div className="flex flex-wrap items-center gap-0.5 px-2 py-1 border-t" style={{ borderColor: 'var(--theme-border)' }}>
          <HeadingDropdown />
          <Divider />
          <ExtraButtons />
        </div>
